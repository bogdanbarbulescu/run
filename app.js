document.addEventListener('DOMContentLoaded', () => {
    ...
    const getInitialData = () => ({
        ...
        prs: {
            longestRun: { value: 0, date: null }, fastest5k: { value: Infinity, date: null },
        },
    ...
    const renderTodayView = () => {
        ...
        const plan = WORKOUT_LIBRARY[appData.plan[todayKey].type];
    ...
    const renderTrophyCase = () => {
        ...
        const trophies = {
            ...
            fastest5k: { icon: '⚡️', title: 'Cel Mai Rapid 5k', value: prs.fastest5k.value === Infinity ? 'N/A' : secondsToPace(prs.fastest5k.value) },
        ...
        document.getElementById('trophyEmptyState').classList.toggle('hidden', Object.values(achievements).some(v=>v));
    };
    ...
    const checkAchievements = (log, dateKey) => {
        ...
        if (distance >= 5 && distance < 6 && timeInSeconds < appData.prs.fastest5k.value) appData.prs.fastest5k = { value: timeInSeconds / distance, date: dateKey };
    ...
    const openModal = (modal, dayKey) => {
        currentModalDayKey = dayKey || formatDateKey(new Date());
        modal.overlay.classList.remove('hidden');
        if (modal === logModal) {
            const log = appData.logs[currentModalDayKey] || {};
            log.form.elements.distanceInput.value = log.distance || '';
            log.form.elements.timeInput.value = log.time || '';
            log.rpeSlider.value = log.rpe || 5;
            log.rpeValue.textContent = log.rpe ? `Efort: ${log.rpe}/10` : 'Selectează o valoare';
        }
    };
    ...
    const timeToSeconds = (timeStr) => { if (!timeStr) return 0; const p = timeStr.split(':').map(Number); return (p[0] || 0) * 3600 + (p[1] || 0) * 60 + (p[2] || 0); };
    const calculatePace = (dist, time) => { if (!dist || !time) return ''; const totalSeconds = timeToSeconds(time); const paceSec = totalSeconds / dist; return secondsToPace(paceSec); };
    const secondsToPace = (sec) => { if (!sec || sec === Infinity) return 'N/A'; const min = Math.floor(sec / 60); const s = Math.round(sec % 60); return `${min}:${s.toString().padStart(2, '0')}`; };
    ...
    const setupEventListeners = () => {
        ...
        mainContent.addEventListener('click', e => {
            ...
            if (action === 'log') openModal(logModal, formatDateKey(new Date()));
        ...
        calendarGrid.addEventListener('click', e => {
            ...
            if (cell) {
                ...
                logDetailModal.body.innerHTML = '...'; // Populate with log details
